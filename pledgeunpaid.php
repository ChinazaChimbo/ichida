<?php
    ...
    if (isset($_GET['id'])) {
      ...
      $in = mysqli_query($con, "UPDATE pledge set paid = TRUE, date = '".$date."' WHERE id = '".$id."'");
      $msg = "<div class = 'alert alert-success'>Pledge has been paid</div>";
    }
    $getpledge = mysqli_query($con, "SELECT * FROM pledge WHERE paid = FALSE ORDER BY id ASC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Ichida Progress Union | All Unpaid pledges</title>
    ...
    while ($a = mysqli_fetch_assoc($getpledge)) {
      ...
      while ($b = mysqli_fetch_assoc($getmember)) {
        ...
        echo "<tr>
                  ...
                  <td>&#8358;".$aamount."</td>
                  <td>".$aphone."</td>
                  <td><a class='btn btn-success' href = 'pledgeunpaid.php?id=$aid'>Paid?</a></td>  
              </tr>";
        $i++;
      }
    }
  } else {
    echo "There are no unpaid pledges";
  }
